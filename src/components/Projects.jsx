      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        >
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${activeFilter === cat ? "bg-gradient-to-r from-blue-500 to-violet-600 text-white shadow-lg shadow-blue-500/25" : "bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700"}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((p, i) => (
            <div
              key={p.id}
              className={`group relative rounded-2xl bg-gray-800/40 border border-gray-700/50 hover:border-gray-600/70 overflow-hidden transition-all duration-500 flex flex-col ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
