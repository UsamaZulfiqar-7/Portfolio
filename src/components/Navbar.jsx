...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-6xl flex items-center justify-between px-5 py-3 rounded-2xl transition-all duration-300 ${scrolled ? "bg-gray-900/90 backdrop-blur-xl border border-gray-700/50 shadow-2xl shadow-black/40" : "bg-transparent"}`}
      >
        <button
          ...
          {links.map(({ href, label }) => (
            <li key={href}>
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-lg font-medium transition-all duration-200 ${active === href.slice(1) ? "text-blue-400 bg-blue-500/10" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}
              >
                {label}
              </button>
            </li>
      ...
      <div
        className={`lg:hidden absolute top-full mt-2 left-4 right-4 bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
      >
